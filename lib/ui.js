...
class App extends React.Component {
  ...
  initSocket(url) {
    ...
    socket.on('connect', () => {
      this.appendMessage(`Conectado con el servidor ${ this.state.url }`);
    ...
    socket.on('login', data => {
      this.appendMessage(`${ data.username } se conecto.`);
      this.setState({ users: data.users });
    });
    socket.on('typing', data => {
      this.setState({ status: `${ data.username } esta escribiendo...` });
    });
    socket.on('stop-typing', () => {
      this.setState({ status: '' });
    });
    socket.on('logout', data => {
      this.appendMessage(`${ data.username } desconectado.`);
      this.setState({ users: data.users });
    });
    this.socket = socket;
  }
  ...
  render() {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('input', { type: 'text', placeholder: 'Ingresa tu Nombre', id: 'username', ref: 'username', onKeyDown: this.onKeyDown, autofocus: true })
      )
    );
  }
}
